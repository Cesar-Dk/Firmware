...
//			script.base.shortName = 0x4F00;
		...
		ret = com_read(con, BASE(script), 2);
		if ( 1 == params_get_slow_meter() )
		{
			HAL_Delay(1000);
		}
...
//			a = (gxScriptAction*)malloc(sizeof(gxScriptAction));
		...
		dlmsVARIANT   params;
		ret = com_method(con, BASE(activityCalendarActiveTime), 1, &params);
...
uint32_t dlms_client_on_demand_comand_get_curr_active_tariff(connection* con)
{
	...
	if ( ( ret = INIT_OBJECT(activityCalendarActiveTime1, DLMS_OBJECT_TYPE_ACTIVITY_CALENDAR, ln1) ) == 0 )
	{
		...
		ret = com_read(con, BASE(activityCalendarActiveTime1), 6);

		if (ret == DLMS_ERROR_CODE_OK)
		{
			char * active_calendar_name = bb_toString(&activityCalendarActiveTime1.calendarNamePassive);
...
//		char *obis = dlms_client_get_obis_value_next();
		if (ret == DLMS_ERROR_CODE_OK)
		{
			sprintf(active_tariff + str_len, ";%d%d#", activeTariff.value.strVal->data[0], activeTariff.value.strVal->data[1]);
		}
		else
		{
