...
//	uint32_t dlms_checksum[32];
	...
	if ( params_get_modbus_type() != LAST_GENERIC_485_TYPE ) {
		sprintf(modbus_params, "%d;%d;%d;%d;%d;%d;%d;%d;%d;%d||",
				...
				(int)generic_485_get_function(),
				(int)generic_485_get_addr(),
				(int)generic_485_get_quantity(),
...
static void __checkNextModbusMessage( void )
{
	...
	if ( ( ( 0 == modbus_sensors_log_CheckDataInMem() ) || ( ( params_get_max_datalogger_msgs() != (uint32_t)(-1) ) && ( num_modbus_frames > params_get_max_datalogger_msgs() ) ) )
