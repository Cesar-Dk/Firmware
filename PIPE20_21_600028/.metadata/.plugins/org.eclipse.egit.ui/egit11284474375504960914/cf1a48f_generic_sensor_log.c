...
#define SENSOR_LOG_BLOCK_SIZE (0x80000)
...
uint32_t generic_sensor_telegram_values_max_address       = (uint32_t) _pressure_start_address + 4*SENSOR_LOG_BLOCK_SIZE;

uint32_t generic_sensor_log_addresses[4] = {(uint32_t) _pressure_start_address,
...
uint32_t generic_sensor_telegram_values_addresses[4] = {(uint32_t) _pressure_start_address + 0x1000,
...
uint32_t generic_sensor_telegram_values_max_addresses[4] = {(uint32_t) _pressure_start_address + 1*SENSOR_LOG_BLOCK_SIZE - 1,
...
generic_sensor_values_t sensor_values[4];
generic_sensor_log_t    sensor_log[4];
...
static uint32_t __checkLowGenericSensorAlarm(uint32_t num)
{
	...
	if (0xFFFFFFFF != params_get_low_sensor_alarm(num))
	{
		...
		{
			if (((AND_CONDITION == params_get_low_sensor_condition()) && (num == NUM_INPUTS_SENSOR-1) && ( 0 == and_condition_break ) )
...
static uint32_t __checkOverGenericSensorAlarm( uint32_t num )
{
	...
	if (0xFFFFFFFF != params_get_over_sensor_alarm(num))
	{
		...
		{
			if (((AND_CONDITION == params_get_over_sensor_condition()) && (num == NUM_INPUTS_SENSOR-1) && ( 0 == and_condition_break ) )
...
char *generic_sensor_log_GetDatalogger_Info(void)
{
	...
	for (i=0; i<NUM_INPUTS; i++)
	{
		if ( 1 == generic_sensor_get_on_off(i) )
		{
...
void generic_sensor_log_pressure_store_telegram_value(uint32_t num)
{
	if (sensor_log[num].init == 0xAAAA)
	{
		...
		LOGLIVE(LEVEL_2, "\r\nLOGLIVE> %d SENSOR LOG> %s%s Pavg:%s. Pinst:%s. Msg.num:%d.\r\n", (int)Tick_Get( SECONDS ),
