...
//							__updateState( 1, MBUS_SLAVES_DISABLE );
	...
	case MBUS_SLAVES_CHANGE_LONG_TELEGRAM:
		if( CHECK_ELAPSED_TIME( mbus_task_get_mbus_delay(), TICK_MBUS ) )
		{
			LOGLIVE(LEVEL_2, "LOGLIVE> %d MBUS> Change Long Telegram Slave num:%d.Slave add:%s.\r\n", (int)Tick_Get( SECONDS ),(int)slave_num, slaves_table->slave[slave_num].secondary);
			if (MBus.attempt_n)
			{
				if (subcode != 0x80)
				{
		...
		case MBUS_PING_RESP:
			if(serial_mbus_rcx_bytes_n())
			{
				...
				if(data[0] == 0xE5)
				{
					...
					if (1 == Tick_cloud_time_init())
					{
						mbus_status = MBUS_DATA_REQ_1;//MBUS_CHANGE_LONG_TELEGRAM;
					}
					else
					{
