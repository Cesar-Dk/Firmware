...
{
    return (uint32_t)(data.average/1000);
}
...
//	if ( ( 1 == _get_adc_data ) && ( 1 == params_pulses_on() ) && ( 1 == pulses_get_pulse_rx() )  ) {
//		get_adc_data = 0;
//		if ( 0 == pulses_get_write_record() ) {
...
{
	if (HAL_ADCEx_Calibration_Start(&hadc4, ADC_CALIB_OFFSET_LINEARITY, ADC_SINGLE_ENDED) != HAL_OK)
	{
		/* Calibration Error */
		Error_Handler();
	}
...
void AD_read_sensor_pressure_sensor(void)
{
	...
	MX_ADC4_DeInit();
	MX_ADC4_Init();
	data.average = (float_t)__read_pressure_sensor();
