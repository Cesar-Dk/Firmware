...
//		mbus_set_watermeter_pr6_manufacturer_serial_num(string_aux);
	...
	if ((udp_get_server_resp_type() == 5) || (udp_get_server_resp_type() == 1) || (udp_get_server_resp_type() == 2))
	{
...
//	}
	...
	while ( (token != NULL) && ( i < NUM_INPUTS_SENSOR ) ) {
		token = strtok( NULL, s1 );
		if (token != NULL) {
			param.over_sensor_alarm[num_sensor-1] = atoi( token );
			token      = strtok( NULL, s );
			if ( token != NULL )
			num_sensor = atoi( token );
			i++;
...
//	}
	...
	while ( (token != NULL) && ( i < NUM_INPUTS_SENSOR ) ) {
		token = strtok( NULL, s1 );
		if (token != NULL) {
			param.low_sensor_alarm[num_sensor-1] = atoi( token );
			token      = strtok( NULL, s );
			if ( token != NULL )
			num_sensor = atoi( token );
			i++;
...
void json_get_modbus_slave_id( char *str )
{
	const char s[2] = ",";
	char *token;
	uint32_t i = 0;
	...
	{
		generic_485_set_slave_id_reg(0, i);
	}
	i = 0;
	token = strtok(str,s);
	while ( token != 0 ) {
	...
	generic_485_set_function_reg(0xFF, i);
}


void json_get_modbus_address_645( char *str )
{
	const char s[2] = ",";
	char *token;
	char bcd[14];
	...
	while ( token != 0 ) {
		...
		for (j = 0; j < 12; j++)
		{
			generic_485_set_address_645(bcd[j], j, i);
		}
...
void json_get_modbus_function( char *str )
{
	const char s[2] = ",";
	char *token;
	uint32_t i = 0;
	...
	token = strtok(str,s);
	while ( token != 0 ) {
		generic_485_set_function_reg(atoi(token), i++);
		modbus_num_commands++;
		token = strtok( NULL, s );
	}
	generic_485_set_function_reg(0xFF, i);
...
void json_get_modbus_address( char *str )
{
	const char s[2] = ",";
	char *token;
	uint32_t i = 0;
	...
	token = strtok(str,s);
	while ( token != 0 ) {
		generic_485_set_data_id_645(atoi(token), i);
		generic_485_set_addr_reg(atoi(token), i++);
...
void json_get_modbus_quantity( char *str )
{
	const char s[2] = ",";
	char *token;
	uint32_t i = 0;
	...
	token = strtok(str,s);
	while ( token != 0 ) {
		generic_485_set_quantity_reg(atoi(token), i++);
		if ( i > modbus_num_commands ) {
			token = 0;
		} else {
			token = strtok( NULL, s );
