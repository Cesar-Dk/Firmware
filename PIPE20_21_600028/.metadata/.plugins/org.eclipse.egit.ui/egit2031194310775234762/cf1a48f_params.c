...
void params_loglive(uint32_t level, const char *f,  ...)
{
	...
	{
		if ( ( modbus_sensors_get_comm_state() != MODBUS_COMM_STARTED ) && (( CHECK_TAMPER_PIN == 0 ) || ( 99 == LOGLIVE_LEVEL )) && ( con_dlms_in_process() == 0 ) && ( ( modbus_get_end_session() == MODBUS_SESSION_END ) || ( 0 == modbus_get_local_tool_started() ) ) ) {
			MX_USART2_UART_Init();
			ENABLE_RS485();
			HAL_Delay(1);
			DE_TX_RS485();
