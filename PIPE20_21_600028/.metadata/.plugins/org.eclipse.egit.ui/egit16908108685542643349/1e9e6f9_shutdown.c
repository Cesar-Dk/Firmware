 ...
 *
 */
static void __throwDLMSRead(void)
{
	static uint32_t minute_backup = (uint32_t)-1;
	/* synch read is enabled but still not the time to read sample */
	if ((synch == 1) && ((shutdown_sync_modbus_read_pressure_on == 0) || (0 == __checkRTCQuart(&minute_backup))))
	...
	else
	{
		...
		dlms_client_set_read_energy_profile(1);
	}
}
#endif
 ...
 *
 */
static void __throwDLMSOndemandRead(void)
{
//	message_queue_write(SEND_MODBUS_SENSOR);
	dlms_client_set_send_meter(1);
	/** Triggers Modbus sensor acquisition if it is the right time */
...
//	/* synch read is enabled but still not the time to read sample */
//	if ((synch == 1) && ((shutdown_sync_modbus_read_pressure_on == 0) || (0 == __checkRTCQuart())))
//	{
//		con_dlms_change_state(&con, DLMS_STATE_IDLE);
//	}
//	else
...
//	leds_LED_Off(LED_WHITE);
	...
	if ( TAMPER_HARD_RESET_DAYLY == shutdown_get_tamper_param())
	{
		if ( 0 == Tick_cloud_time_init() )
		{
			Tick_system_time_init(rtc_system_get_t_seconds_backup());//Tick_system_time_init(params_get_time_reset());
...
//						rtc_system_SetPulseAlarm();
	...
	{
		if ( ( asleep_time != 2 ) && ( 1 == mqtt_timer_get_idle_mode() ) )
		{
...
//		rtc_system_ReconfigStopMode(); //TOASK hace falta?
		...
		HAL_PWREx_EnableRAMsContentStopRetention(PWR_SRAM1_FULL_STOP|PWR_SRAM2_FULL_STOP|PWR_SRAM3_FULL_STOP|PWR_SRAM4_FULL_STOP|PWR_ICACHE_FULL_STOP|PWR_DMA2DRAM_FULL_STOP);
			...
			if (1 == Tick_cloud_time_init())
			{
				if (ALARM_SEND == rtc_system_getCurrentAlarmFromBKUP())
				{
					if (1 == __checkRTCAlarm( rtc_system_getSendAlarmSendNext()))
					{
						shutdown_setInitTelitModule(1);
//						if ( 1 == params_pulses_on() ) {
//							if ( 1 == Datalogger_CheckLIFOPointers() ) {
...
//			une82326_set_start_comm(1);     //DEBUG: Testing UNE.
//			mbus_set_start_comm(1);         //DEBUG: Testing MBUS.
//			AD_SetGetADCData(1);            //DEBUG: Testing Pressure.

//			aqualabo_modbus_enable(1);      //DEBUG: Testing Aqualabo.
//			modbus_sensors_set_get_data(1); //DEBUG: Testing Aqualabo.
//			aqualabo_set_comm_state(1);     //DEBUG: Testing Aqualabo.
//			shutdown_setInitTelitModule(0); //DEBUG: Testing Aqualabo.

//			pulses_ch2p_handler();          //DEBUG: Testing Pulses.
			if (1 == rtc_system_getAlarmOccured()) {
				rtc_system_setAlarmOccured(0);
			}
			if ( 1 == rtc_system_getAlarmProgrammedReset() ) {
//				HAL_NVIC_SystemReset();
				rtc_system_setAlarmProgrammedReset(0);
				rtc_system_Set( 2 );
				HAL_PWR_EnterSTANDBYMode();
			}
...
static void __setCounter(int32_t * minute_factor, int32_t * minute_factor_modbus, int32_t * minute_factor_dlms)
{
	...
	if ((1 == AD_GetADOn()))
	{
		...
		if (0 != generic_485_get_read_time())
		{
			...
			{
				*minute_factor        = (int32_t) (generic_485_get_read_time() / div);
				*minute_factor_modbus = (int32_t) (generic_485_get_read_time() / div);

				if (0 == modbus_count)
				{
					modbus_count  = (int32_t) ((generic_485_get_send_time() / generic_485_get_read_time()) * (*minute_factor));
				}
			}
					...
					*minute_factor_dlms   = (int32_t) (dlms_client_get_read_time() / div);
					...
					for (j = 0; j < dlms_client_table_get_num_devices(); j++)
					{
						...
						for (i = 0; i < DLMS_PROFILE_NUM; i++)
						{
							if (0 == dlms_count[j][i])
							{
								dlms_count[j][i] = (int32_t) ((dlms_client_get_send_time_obis_n(i) / dlms_client_get_read_time_obis_n(i)) * (*minute_factor));
							}
						}
					}
				}
					...
					*minute_factor_dlms   = (int32_t) (param.config.rt / div);
					...
					for (j = 0; j < dlms_client_table_get_num_devices(); j++)
					{
						...
						for (i=0; i < DLMS_PROFILE_NUM; i++)
						{
							if (0 == dlms_count[j][i])
							{
								dlms_count[j][i]  = (int32_t) ((dlms_client_get_send_time_obis_n(i) / param.config.rt) * (*minute_factor_dlms));
							}
						}
					}
				}
					...
					for (j = 0; j < dlms_client_table_get_num_devices(); j++)
					{
						...
						for (i = 0; i < DLMS_PROFILE_NUM; i++)
						{
							if (0 == dlms_count[j][i])
							{
								...
								{
									dlms_count[j][i] = (int32_t) (dlms_client_get_send_time_obis_n(i) / dlms_client_get_read_time_obis_n(i));
								}
							}
					...
					for (j = 0; j < dlms_client_table_get_num_devices(); j++)
					{
						...
						for (i=0; i < DLMS_PROFILE_NUM; i++)
						{
							if (0 == dlms_count[j][i])
							{
								...
								dlms_count[j][i] = (int32_t) (dlms_client_get_send_time_obis_n(i) / rt);
							}
						}
					}
...
static uint32_t __checkCounters(uint32_t client_id, uint32_t index, int32_t *counter, int32_t minute_factor)
{
	dlms_client_table_read_client_id( dlms_client_get_client(), client_id );

	if (index<8)
	{
	dlms_client_table_read_client_obis_profile(client_id, dlms_client_get_client(),  dlms_client_get_client_obis_profile(), index);
	}
	else
	{
		dlms_client_table_read_client_generic_profile(client_id, dlms_client_get_client(), dlms_client_get_client_generic_profile(), index);
	}
	...
	else
	{
		...
		if ((dlms_client_get_read_time() != 0)
				&& ( (dlms_client_get_read_time_obis_n(index)!=0) && ((div_mod != 0) && (*counter % (div_mod)) == 0 ) /*|| (shutdown_sync_modbus_read_pressure_on == 1)*/)
				&& (1 == Tick_cloud_time_init())
		)
		{
			LOGLIVE(LEVEL_2, "LOGLIVE> %d CIRCULAR_BUFFER> Read Queue.\r\n", (int)Tick_Get( SECONDS ));
...
//						__DlmsSchedule( minute_factor_dlms, shutdown_time_periodic_mode );
			...
			message_queue_write( SEND_SENSOR );//DEBUG:Testing Sensor Plus Frame.
